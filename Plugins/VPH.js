...
Trex.addrex({pattern: 'uploadv ?(.*)', fromMe: true,  deleteCommand: true,  desc: 'short cut'}, (async (message, match) => {    
    ...
    if (!message.reply_message) return await message.client.sendMessage(message.jid, 'මෙගාබයිට් පහට අඩු වීඩියෝ එකකට මෙන්ශන් කරන්න ( Mention Video Less than 5mb ) 🖲️', MessageType.text);
...
var location = await message.client.downloadAndSaveMediaMessage({
    ...
    ffmpeg(location)
        ...
        var geturl = await uploadByBuffer(fs.readFileSync('Dragonx.mp4' ), 'video/mp4')
